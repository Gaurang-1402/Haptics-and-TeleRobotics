...
void loop() {
  ...
  // Virtual spring section
   float x0 = -0.0410;
   int k = 6;
   force = -k * (xh-x0);
 
  // Virtual wall section
  int force_estimate = 2;
  
  float x_wall = 0.0025;
  
  if (xh > x_wall) {
     force = force_estimate * (x_wall - xh); 
  } else {
    force = 0; 
  }
  ...
  double dT = 0.0001; //estimated loop time
  
  vh = (((xh - lastXh)/dT) + lastVh + lastLastVh)/3;
  lastXh = xh; //update the previous values with the current 
  lastLastVh = lastVh;
  lastVh = vh;
